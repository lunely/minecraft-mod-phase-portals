...
public record InterdimensionalTeleportFrequencySnapshotPayload(int syncId, String assigned, boolean assignedPrivate,
    ...
    public static final Id<InterdimensionalTeleportFrequencySnapshotPayload> ID =
            new Id<>(Identifier.of(PhasePortalsMod.MOD_ID, "interdimensional_frequency_snapshot"));
    public static final PacketCodec<RegistryByteBuf, InterdimensionalTeleportFrequencySnapshotPayload> CODEC = PacketCodec.of(
            (payload, buf) -> {
                buf.writeVarInt(payload.syncId);
                buf.writeString(payload.assigned, InterdimensionalFrequencyState.MAX_NAME_LENGTH);
            ...
            buf -> {
                ...
                String assigned = buf.readString(InterdimensionalFrequencyState.MAX_NAME_LENGTH);
                ...
                return new InterdimensionalTeleportFrequencySnapshotPayload(syncId, assigned, assignedPrivate, assignedHidden,
    ...
    private static void writeList(RegistryByteBuf buf, List<String> names, List<Integer> colors,
        ...
        for (int i = 0; i < names.size(); i++) {
            buf.writeString(names.get(i), InterdimensionalFrequencyState.MAX_NAME_LENGTH);
    ...
    private static void readList(RegistryByteBuf buf, List<String> names, List<Integer> colors,
        ...
        for (int i = 0; i < count; i++) {
            names.add(buf.readString(InterdimensionalFrequencyState.MAX_NAME_LENGTH));
