...
public final class InterdimensionalTeleportScreen extends HandledScreen<InterdimensionalTeleportScreenHandler> {
    ...
    private int currentColor = InterdimensionalFrequencyState.DEFAULT_COLOR;
    ...
    protected void init() {
        ...
        nameField.setMaxLength(InterdimensionalFrequencyState.MAX_NAME_LENGTH);
    ...
    private void createFrequency() {
        String name = InterdimensionalFrequencyState.normalize(nameField.getText());
        if (name.isEmpty()) return;
        selected = name;
        send(InterdimensionalTeleportFrequencyActionPayload.CREATE, name, 0, privateTab);
        nameField.setText("");
    }

    private void setFrequency() {
        if (!selected.isEmpty() && names().contains(selected))
            send(InterdimensionalTeleportFrequencyActionPayload.SET, selected, 0, privateTab);
    }
    ...
    private void confirmDelete() {
        ...
        send(InterdimensionalTeleportFrequencyActionPayload.DELETE, pendingDelete, 0, pendingPrivate);
        confirmDelete = false;
        pendingDelete = "";
    }
        ...
        if (button == 0 && paletteOpen) {
            for (int color = 0; color < PortalColors.count(); color++) {
                ...
                if (mouseX >= swatchX && mouseX < swatchX + 14
                    ...
                    send(InterdimensionalTeleportFrequencyActionPayload.COLOR, assigned, color, assignedPrivate);
                    paletteOpen = false;
                    return true;
                }
            }
    ...
    public void render(DrawContext context, int mouseX, int mouseY, float delta) {
        createButton.active = !InterdimensionalFrequencyState.normalize(nameField.getText()).isEmpty();
